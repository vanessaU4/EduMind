...
const ContentManagementDashboard: React.FC = () => {
  ...
  const fetchAllContent = async () => {
    ...
    try {
      const [articlesResponse, videosResponse, audioResponse, resourcesResponse] = await Promise.all([
      ...
      setArticles(extractArrayFromResponse(articlesResponse));
      setVideos(extractArrayFromResponse(videosResponse));
      setAudioContent(extractArrayFromResponse(audioResponse));
      setResources(extractArrayFromResponse(resourcesResponse));
    } catch (error) {
  ...
  const handleEditSave = (updatedContent: Article | VideoType | AudioContent | MentalHealthResource) => {
    ...
    fetchContentStats(); // Refresh stats
  };

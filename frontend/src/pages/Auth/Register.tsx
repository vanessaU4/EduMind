...
const Register: React.FC = () => {
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    birthDate: '', // Single field for complete birth date
    role: 'user',
  });
  ...
  // Date range for ages 13-23
  const currentYear = new Date().getFullYear();
  const minDate = `${currentYear - 23}-01-01`; // 23 years old (oldest)
  const maxDate = `${currentYear - 13}-12-31`; // 13 years old (youngest)
  ...
  const validateForm = () => {
    ...
    if (!formData.birthDate) {
      ...
      if (age < 13 || age > 23) {
        newErrors.birthDate = 'This platform is designed for youth aged 13-23';
      }
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              <input
                ...
                min={minDate}
                max={maxDate}
                className="h-12 w-full px-4 border border-gray-200 rounded-md focus:border-healthcare-primary focus:ring-healthcare-primary focus:ring-1 focus:outline-none transition-all duration-200 bg-white"
                required
              />
              {errors.birthDate && (
                <motion.p 
